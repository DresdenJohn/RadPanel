...
Partial Class WebMaster
    ...
    Protected Sub Page_PreRender(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRender
        ...
        Dim errorLog As String = Request.QueryString("err")

        Select Case (errorLog)
            Case "isndfl"
                sendNotification("SendItem Failed...please contact an administrator.")
        End Select
    ...
    Public Sub runStartupFunctions(ByVal isLoggedIn As Boolean)
        ...
        If isLoggedIn = False Then
            usernameTopBoxLabel.Text = "<span>You are not signed in.</span>"
            loginPlaceHolder.Visible = True
            'sendNotification("I noticed you&#39;re not logged in!<br/>Why not consider joining? It&#39;s <a class=fancybox  href=register.aspx>free</a>!")
        Else
            If webFunc.getAccountAuth(g_email) = "S" Or webFunc.getAccountAuth(g_email) = "A" Then
                adminLinksPlaceHolder.Visible = True
            End If
            accountLinksPlaceHolder.Visible = True
            g_username = g_username.Remove(g_username.Length - 1, 1)
            usernameTopBoxLabel.Text = "<span>Signed in as: " & webFunc.getUsername(g_email) & "</span>"
            loggedInTopPlaceholder.Visible = True
            loggedOutTopPlaceholder.Visible = False

        End If

    End Sub

    Public Sub updateIrisStatus()
        ...
        irisGuildCount.Text = flyffFunc.countFlyffGuilds()

        irisAccountNumber.Text = webFunc.countGameAccounts("Flyff", g_username)
    ...
    Protected Sub loginButton_Click(sender As Object, e As EventArgs)
        ...
        Session("emailText") = loginUsernameText.Text
        Session("passwordText") = loginPasswordText.Text
        Session("rememberMe") = rememberMeLoginCheckbox.Checked
